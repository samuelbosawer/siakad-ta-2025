        ...
        <div class="col-12">
            <div class="card mb-4">
                ...
                <div class="card-body">
                    ...
                    @if($isUbah)
                        <form action="{{ route('dashboard.form-magang.update', $data->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                    @elseif($isTambah)
                    <form action="{{ route('dashboard.form-magang.store') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                    @else
                        <form>
                    @endif
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select name="mahasiswa_id" class="form-select" @if($isDetail) disabled @endif>
                         ...
                         <div class="col-md-4 mb-3">
                            ...
                            <select name="magang_id" class="form-select" @if($isDetail) disabled @endif>
                                <option value="">Pilih Magang</option>
                                @foreach ($magangs as $m)
                                    <option value="{{ $m->id }}"
                                        @selected( old('magang_id', $data->magang_id ?? '') == $m->id )>
                                        {{ $m->nama_magang }}
                                    </option>
                                @endforeach
                            </select>
                            @error('magang_id') <small class="text-danger">{{ $message }}</small> @enderror
                        </div>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="date" class="form-control" name="tanggal"
                                value="{{ old('tanggal', $data->tanggal ?? '') }}" @if($isDetail) disabled @endif>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            @if(isset($data) && $data->berkas)
                                <p>
                                    <a class="btn btn-dark btn-sm" target="_blank" href="{{ asset('storage/' . $data->berkas) }}">
                                        Lihat File
                                    </a>
                                </p>
                            @endif
                            ...
                            @error('sk') <small class="text-danger">{{ $message }}</small> @enderror
                        </div>

                        {{-- STATUS --}}
                        <div class="col-md-6 mb-3">
                            <label class="form-label">Status</label>
                            <select name="status" class="form-select" @if($isDetail) disabled @endif>
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            @error('ket') <small class="text-danger">{{ $message }}</small> @enderror
                        </div>
