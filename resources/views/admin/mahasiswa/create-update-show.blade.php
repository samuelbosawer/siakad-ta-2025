                ...
                <div class="row">
                    <div class="card mb-4">
                        <h5 class="card-header fw-bolder"><i class="menu-icon tf-icons bx bx-user"></i>
                            {{ $judul ?? 'TAMBAH DATA MAHASISWA' }} </h5>
                        <div class="card-body">

                            @if (Request::segment(4) == 'ubah' && Request::segment(2) == 'mahasiswa')
                                <form action="{{ route('dashboard.mahasiswa.update', $data->id) }}"
                                    enctype="multipart/form-data" method="POST">
                                    @csrf
                                    @method('PUT')
                                @elseif (Request::segment(3) == 'tambah' && Request::segment(2) == 'mahasiswa')
                                    <form action="{{ route('dashboard.mahasiswa.store') }}" enctype="multipart/form-data"
                                        method="POST">
                                        @csrf
                                    @else
                                        <form action="">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" class="form-control" id="nama_depan" name="nama_depan"
                                        ...
                                        @if (Auth::user()->HasRole('mahasiswa')) disabled @endif>
                                    @error('nama_depan')
                                ...
                                <div class="col-md-12 mb-3">
                                    ...
                                    <textarea class="form-control" id="alamat" name ="alamat" rows="3"
                                        @if (Request::segment(3) == 'detail') disabled @endif @if (Auth::user()->HasRole('mahasiswa')) disabled @endif>{{ old('alamat') ?? ($data->alamat ?? '') }}</textarea>
                                    @error('alamat')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-12 mb-3 mx-auto">
                                @if (Request::segment(3) == 'detail')
                                    <a href="{{ route('dashboard.mahasiswa.ubah', $data->id) }}"
                                ...
                                <a href="{{ route('dashboard.mahasiswa') }}" class="btn btn-dark text-white"> KEMBALI
                                </a>
